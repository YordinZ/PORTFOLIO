...
/* ===================== SKILLS DATA ===================== */
const skills: Skill[] = [
  { name: "Python", icon: <PythonIcon />, category: "Dev", description: "Primary programming language for data analysis, automation, and machine learning", orbit: 1 },
  { name: "SQL", icon: <SQLIcon />, category: "Data", description: "Database querying, optimization, and relational data management", orbit: 1 },
  { name: "Pandas", icon: <PandasIcon />, category: "Data", description: "Data manipulation, cleaning, and structured data analysis", orbit: 1 },
  { name: "NumPy", icon: <NumPyIcon />, category: "Data", description: "Numerical computing and multidimensional array operations", orbit: 1 },

  { name: "JavaScript", icon: <JavaScriptIcon />, category: "Dev", description: "Interactive web development and modern ES6+ features", orbit: 2 },
  { name: "HTML", icon: <HTMLIcon />, category: "Dev", description: "Semantic web structure and accessibility best practices", orbit: 2 },
  { name: "React", icon: <ReactIcon />, category: "Dev", description: "Modern UI development using reusable component architecture", orbit: 2 },
  { name: "Tailwind CSS", icon: <TailwindIcon />, category: "Dev", description: "Utility-first CSS framework for rapid and scalable UI design", orbit: 2 },

  { name: "Git", icon: <GitIcon />, category: "Tools", description: "Version control and collaborative development workflows", orbit: 2 },
  { name: "GitHub", icon: <GitHubIcon />, category: "Tools", description: "Repository management, collaboration, and CI/CD integration", orbit: 2 },
  { name: "Docker", icon: <DockerIcon />, category: "Tools", description: "Containerization and environment consistency for deployment", orbit: 2 },

  { name: "Power BI", icon: <PowerBIIcon />, category: "Viz", description: "Interactive dashboards and business intelligence reporting", orbit: 2 },
  { name: "Streamlit", icon: <StreamlitIcon />, category: "Viz", description: "Interactive data applications and rapid ML prototyping", orbit: 2 },

  { name: "Machine Learning Fundamentals", icon: <MLIcon />, category: "ML", description: "Supervised and unsupervised learning concepts and model evaluation", orbit: 3 },
  { name: "Statistics", icon: <StatsIcon />, category: "Data", description: "Probability theory, hypothesis testing, and statistical inference", orbit: 3 },
  { name: "Model Validation", icon: <ValidationIcon />, category: "ML", description: "Performance metrics, cross-validation, and model evaluation techniques", orbit: 3 },
  { name: "REST APIs", icon: <APIIcon />, category: "Dev", description: "Integration and development of RESTful services", orbit: 3 },
  { name: "Matplotlib", icon: <MatplotlibIcon className="w-6 h-6" />, category: "Data", description: "Data visualization and exploratory analysis using Python", orbit: 3 },
  { name: "Atlassian (Jira / Confluence)", icon: <AtlassianIcon />, category: "Tools", description: "Agile project management and technical documentation", orbit: 3 },
  { name: "QA/QC", icon: <QAIcon />, category: "Tools", description: "Quality assurance, testing methodologies, and process validation", orbit: 3 },
  { name: "PostgreSQL", icon: <PostgreSQLIcon />, category: "Data", description: "Relational database design, optimization, and data integrity", orbit: 3 },
  { name: "Neon", icon: <NeonIcon />, category: "Data", description: "Serverless PostgreSQL for scalable cloud-based applications", orbit: 3 },
  ...
  { name: "Formspree", icon: <FormspreeIcon />, category: "Tools", description: "Form backend service for handling submissions without a custom server", orbit: 3 },
  ...
  { name: "Hugging Face", icon: <HuggingFaceIcon className="w-6 h-6" />, category: "ML", description: "Open-source AI ecosystem for transformers, NLP, and ML model deployment", orbit: 3 },
];

const getCategoryLabel = (id: string, allLabel: string) => {
  if (id === "all") return allLabel;
  const map: Record<string, string> = { Data: "DATA", Dev: "DEV", ML: "ML", Tools: "TOOLS", Viz: "VIZ" };
  return map[id] || id;
};

const categoryIds = ["all", "Data", "Dev", "ML", "Tools", "Viz"] as const;
...
export default function Skills() {
  ...
  const [activeCategory, setActiveCategory] =
    useState<(typeof categoryIds)[number]>("all");
  ...
  return (
    <section
      ref={sectionRef}
      id="skills"
      className="section-page relative overflow-x-clip overflow-y-visible flex flex-col justify-center"
    >
      {/* ========= Aura + particles (se mantiene, pero sin fondo propio) ========= */}
      ...
      <div className="absolute inset-x-0 top-32 bottom-0 pointer-events-none">
        {particles.map((p) => (
          <div
            key={p.id}
      ...
      <motion.div className="relative z-10 mx-auto max-w-7xl px-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-12 gap-6">
          <div className="space-y-3">
            <h2 className="text-5xl md:text-6xl font-black tracking-tight text-foreground font-display">
              {t.skillsTitle}{" "}
              <span className="gradient-text">{t.skillsTitleAccent}</span>
            </h2>
            <div className="gradient-line w-32" />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {categoryIds.map((id) => (
              ...
              >
                {getCategoryLabel(id, t.skillsCatAll)}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="relative flex items-center justify-center">
          ...
          >
            <motion.div className="absolute inset-0">
              ...
              >
                <circle cx="0" cy="0" r={orbitRadii[1]} fill="none" stroke="rgba(34,211,238,0.25)" strokeWidth="1.8" />
                <circle cx="0" cy="0" r={orbitRadii[2]} fill="none" stroke="rgba(139,92,246,0.25)" strokeWidth="1.8" />
                <circle cx="0" cy="0" r={orbitRadii[3]} fill="none" stroke="rgba(232,121,249,0.25)" strokeWidth="1.8" />
              </svg>
              ...
              <motion.div
                ...
                style={{
                  ...
                  background: "linear-gradient(135deg, rgb(34, 211, 238), rgb(232, 121, 249))",
                  boxShadow: "0 0 40px rgba(34, 211, 238, 0.4), 0 0 80px rgba(232, 121, 249, 0.3)",
                }}
              >
                <span className="text-3xl font-black text-white drop-shadow-lg">YH</span>
              </motion.div>

              {/* Nodes */}
              <AnimatePresence mode="sync" initial={false}>
                {getSkillsByOrbit(1).map((skill, index, arr) => {
                  const pos = getPositionOnOrbit(index, arr.length, orbitRadii[1], -Math.PI / 4);
                  return (
                    <SkillNode
                      key={skill.name}
                      skill={skill}
                ...
                {getSkillsByOrbit(2).map((skill, index, arr) => {
                  const pos = getPositionOnOrbit(index, arr.length, orbitRadii[2], Math.PI / 6);
                  return (
                    <SkillNode
                      key={skill.name}
                      skill={skill}
                ...
                {getSkillsByOrbit(3).map((skill, index, arr) => {
                  const pos = getPositionOnOrbit(index, arr.length, orbitRadii[3], 0);
                  return (
                    <SkillNode
                      key={skill.name}
                      skill={skill}
...
function SkillNode({
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.5, delay: hasAnimated ? 0 : delay, type: "spring", stiffness: 150 }}
